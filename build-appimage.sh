...
# Use Python 3.10 for AppImage compatibility with Ubuntu 22.04
# (venv may use 3.11 but the AppImage will bundle 3.10 from Ubuntu)
PYTHON_VERSION="3.10"
print_status "Using Python ${PYTHON_VERSION} for Ubuntu 22.04 compatibility"

# Copy Python executable - use Ubuntu binaries if available for glibc 2.35 compatibility
if [ -f "/tmp/ubuntu-python/python${PYTHON_VERSION}" ]; then
...
# Use Ubuntu binaries if available for glibc 2.35 compatibility
print_status "Bundling Python shared library..."
mkdir -p AppDir/usr/lib
if [ -f "/tmp/ubuntu-python/libpython${PYTHON_VERSION}.so.1.0" ]; then
...
if [ -d "/tmp/ubuntu-python/python${PYTHON_VERSION}-stdlib" ]; then

...
# AppImage Build Script for TalkType
# This creates a self-contained AppImage with all dependencies bundled
...
# Use Python 3.11 for AppImage (available on this system)
# (venv may use 3.13 but the AppImage will bundle 3.11 from system)
# Can be overridden with PYTHON_VERSION_OVERRIDE for Ubuntu 20.04 builds
PYTHON_VERSION="${PYTHON_VERSION_OVERRIDE:-3.11}"
if [ -n "$PYTHON_VERSION_OVERRIDE" ]; then
    print_status "Using Python ${PYTHON_VERSION} (overridden for Ubuntu 20.04 compatibility)"
else
    print_status "Using Python ${PYTHON_VERSION} for Ubuntu 22.04 compatibility"
fi

# Copy Python executable - use Ubuntu binaries if available for glibc compatibility
if [ "$USE_UBUNTU20_PYTHON" = "yes" ] && [ -f "python${PYTHON_VERSION}-ubuntu20" ]; then
    cp "python${PYTHON_VERSION}-ubuntu20" AppDir/usr/bin/python3
    print_status "Copied Python from Ubuntu 20.04 binaries (glibc 2.31 compatible)"
elif [ -f "/tmp/ubuntu-python/python${PYTHON_VERSION}" ]; then
    cp "/tmp/ubuntu-python/python${PYTHON_VERSION}" AppDir/usr/bin/python3
    print_status "Copied Python from Ubuntu 22.04 binaries (glibc 2.35 compatible)"
elif [ -f "/usr/bin/python${PYTHON_VERSION}" ]; then
    cp -L "/usr/bin/python${PYTHON_VERSION}" AppDir/usr/bin/python3
    print_status "Copied Python from /usr/bin/python${PYTHON_VERSION}"
elif [ -f "/usr/bin/python3" ]; then
    cp -L "/usr/bin/python3" AppDir/usr/bin/python3
    print_status "Copied Python from /usr/bin/python3"
else
    print_error "Could not find Python binary!"
    exit 1
fi
chmod +x AppDir/usr/bin/python3

# Verify the Python binary's glibc requirements
GLIBC_NEEDED=$(readelf -V AppDir/usr/bin/python3 2>/dev/null | grep "GLIBC_" | sort -u | tail -1 | awk '{print $2}')
print_status "Python binary requires: ${GLIBC_NEEDED}"

# Bundle Python shared library (CRITICAL for AppImage to work)
# Use Ubuntu binaries if available for glibc compatibility
print_status "Bundling Python shared library..."
mkdir -p AppDir/usr/lib
if [ "$USE_UBUNTU20_PYTHON" = "yes" ] && [ -f "libpython${PYTHON_VERSION}.so.1.0-ubuntu20" ]; then
    cp "libpython${PYTHON_VERSION}.so.1.0-ubuntu20" AppDir/usr/lib/libpython${PYTHON_VERSION}.so.1.0
    print_status "Copied libpython from Ubuntu 20.04 binaries (glibc 2.31 compatible)"
elif [ -f "/tmp/ubuntu-python/libpython${PYTHON_VERSION}.so.1.0" ]; then
    cp "/tmp/ubuntu-python/libpython${PYTHON_VERSION}.so.1.0" AppDir/usr/lib/
    print_status "Copied libpython from Ubuntu 22.04 binaries (glibc 2.35 compatible)"
...
elif [ -f "/usr/lib/libpython${PYTHON_VERSION}.so.1.0" ]; then
    cp "/usr/lib/libpython${PYTHON_VERSION}.so.1.0" AppDir/usr/lib/
    print_status "Copied libpython${PYTHON_VERSION}.so.1.0 from /usr/lib"
elif [ -f "/lib64/libpython${PYTHON_VERSION}.so.1.0" ]; then
...
# Verify libpython's glibc requirements
LIBPYTHON_GLIBC=$(readelf -V "AppDir/usr/lib/libpython${PYTHON_VERSION}.so.1.0" 2>/dev/null | grep "GLIBC_" | grep -v "GLIBC_PRIVATE" | sort -u | tail -1 | awk '{print $2}')
print_status "libpython requires: ${LIBPYTHON_GLIBC}"

# Bundle Python dependencies (needed by Python itself)
print_status "Bundling Python dependencies..."
for libpath in /lib64 /usr/lib64 /usr/lib/x86_64-linux-gnu /usr/lib; do
    if [ -f "$libpath/libexpat.so.1" ]; then
        cp -L "$libpath/libexpat.so.1" AppDir/usr/lib/
        print_status "Copied libexpat.so.1 (XML parser for Python)"
        break
    fi
done
...
if [ "$USE_UBUNTU20_PYTHON" = "yes" ] && [ -d "python${PYTHON_VERSION}-stdlib-ubuntu20" ]; then
    rsync -a "python${PYTHON_VERSION}-stdlib-ubuntu20/" "AppDir/usr/lib/python${PYTHON_VERSION}/"
    print_status "Copied Python stdlib from Ubuntu 20.04"
elif [ -d "/tmp/ubuntu-python/python${PYTHON_VERSION}-stdlib" ]; then
    rsync -a "/tmp/ubuntu-python/python${PYTHON_VERSION}-stdlib/" "AppDir/usr/lib/python${PYTHON_VERSION}/"
    print_status "Copied Python stdlib from Ubuntu 22.04"
elif [ -d "/usr/lib/python${PYTHON_VERSION}" ]; then
...
# Note: venv may use Python 3.11, but we're bundling as 3.10 (compatible)
print_status "Copying Python packages (excluding PyTorch and unnecessary packages)..."
VENV_PYTHON_VERSION=$(.venv/bin/python3 -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')")
print_status "Copying packages from venv Python ${VENV_PYTHON_VERSION} to AppImage Python ${PYTHON_VERSION}"
...
rsync -a --exclude='__pycache__' \
    ...
    --exclude='wheel' \
    --exclude='wheel-*' \
    ".venv/lib/python${VENV_PYTHON_VERSION}/site-packages/" \
...
if [ -d ".venv/lib64/python${VENV_PYTHON_VERSION}/site-packages" ]; then
    ...
    rsync -a --exclude='__pycache__' \
        ...
        --exclude='pip' \
        --exclude='pip-*' \
        ".venv/lib64/python${VENV_PYTHON_VERSION}/site-packages/" \
        "AppDir/usr/lib/python${PYTHON_VERSION}/site-packages/"
fi
...
if [ -d ".venv/lib64/python${VENV_PYTHON_VERSION}/site-packages/torch" ]; then
    cp -r ".venv/lib64/python${VENV_PYTHON_VERSION}/site-packages/torch" \
          "AppDir/usr/lib/python${PYTHON_VERSION}/site-packages/"
    cp -r ".venv/lib64/python${VENV_PYTHON_VERSION}/site-packages/torch-"*.dist-info \
    ...
    cp -r ".venv/lib/python${VENV_PYTHON_VERSION}/site-packages/torch" \
          "AppDir/usr/lib/python${PYTHON_VERSION}/site-packages/" 2>/dev/null || true
fi
...
# Bundle GTK3 and GObject Introspection dependencies (CRITICAL for system tray to work)
# The AppImage MUST be self-contained and cannot rely on host system GTK libraries
print_status "Bundling GTK3 and GObject Introspection libraries..."

# Find system library path by checking where libgtk actually is
if [ -f "/usr/lib/x86_64-linux-gnu/libgtk-3.so.0" ]; then
    SYSLIB="/usr/lib/x86_64-linux-gnu"
elif [ -f "/usr/lib64/libgtk-3.so.0" ]; then
    SYSLIB="/usr/lib64"
elif [ -f "/usr/lib/libgtk-3.so.0" ]; then
    SYSLIB="/usr/lib"
else
    print_error "Could not find GTK3 libraries!"
    exit 1
fi

print_status "Copying GTK3 and GObject Introspection libraries from $SYSLIB..."

# Copy essential GTK3 and GObject libraries (the missing dependencies that caused CI failure)
mkdir -p AppDir/usr/lib

# GObject Introspection (THE CRITICAL MISSING LIBRARY)
# Ubuntu 22.04 has version 1.0, but newer PyGObject expects 2.0
# Try to copy 2.0 first, fall back to 1.0, then create compatibility symlink
if [ -f "$SYSLIB/libgirepository-2.0.so.0" ]; then
    cp -L "$SYSLIB/libgirepository-2.0.so.0" AppDir/usr/lib/
elif [ -f "$SYSLIB/libgirepository-1.0.so.1" ]; then
    cp -L "$SYSLIB/libgirepository-1.0.so.1" AppDir/usr/lib/
    # Create symlink for PyGObject 3.52+ which expects version 2.0
    ln -sf libgirepository-1.0.so.1 AppDir/usr/lib/libgirepository-2.0.so.0
    print_status "Created compatibility symlink: libgirepository-2.0.so.0 -> libgirepository-1.0.so.1"
fi

# GTK3 core
for lib in libgtk-3.so.0 libgdk-3.so.0; do
    [ -f "$SYSLIB/$lib" ] && cp -L "$SYSLIB/$lib" AppDir/usr/lib/ || true
done

# GObject and GLib core (already partially included, but ensure all are there)
for lib in libgobject-2.0.so.0 libgio-2.0.so.0 libglib-2.0.so.0 libgmodule-2.0.so.0; do
    [ -f "$SYSLIB/$lib" ] && cp -L "$SYSLIB/$lib" AppDir/usr/lib/ || true
done

# Cairo and Pango (rendering)
for lib in libcairo.so.2 libcairo-gobject.so.2 libpango-1.0.so.0 libpangocairo-1.0.so.0; do
    [ -f "$SYSLIB/$lib" ] && cp -L "$SYSLIB/$lib" AppDir/usr/lib/ || true
done
...
# ATK (accessibility)
for lib in libatk-1.0.so.0 libatk-bridge-2.0.so.0; do
    [ -f "$SYSLIB/$lib" ] && cp -L "$SYSLIB/$lib" AppDir/usr/lib/ || true
done

# Additional dependencies
for lib in libffi.so.8 libepoxy.so.0; do
    [ -f "$SYSLIB/$lib" ] && cp -L "$SYSLIB/$lib" AppDir/usr/lib/ || true
done

# Copy GObject Introspection typelibs (needed for PyGObject)
print_status "Copying GObject Introspection typelibs..."
mkdir -p AppDir/usr/lib/girepository-1.0
if [ -d "$SYSLIB/girepository-1.0" ]; then
    cp "$SYSLIB/girepository-1.0"/Gtk-3.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/Gdk-3.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/GObject-2.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/GLib-2.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/Gio-2.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/GdkPixbuf-2.0.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
    cp "$SYSLIB/girepository-1.0"/AppIndicator3-0.1.typelib AppDir/usr/lib/girepository-1.0/ 2>/dev/null || true
fi

print_success "GTK3 and GObject Introspection libraries bundled successfully"
